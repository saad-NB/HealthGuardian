...
- Two-tier triage engine:
  - **Tier 1** — deterministic, rule-based, offline, sub-second: GCS, NEWS2, burn TBSA/degree (via guided UI, not image classification), and a hard red-flag override layer.
...
- On-device vision input (skin/eye/throat classifiers) is **not** in scope — dropped per `docs/DECISIONS.md` ADR-020; the app's input surface is patient-reported data + phone sensors (camera PPG, microphone).
...
- ~~Drug interaction / adverse-effect checking~~ — shipped (see `docs/DRUG_INTERACTIONS.md`, ADR-018).
...
| On-device vitals sensing | **`camera` + `record`** | Heart rate via camera PPG, breathing rate via microphone (ADR-017) |
...
/docs
  ...
  DECISIONS.md          — architecture decision records (ADRs). One entry per significant design choice
                           (e.g., "why text-only MedGemma," "why vision input was dropped," "why fllama over
  ...
  MODEL_CARDS/          — one file per model (MedGemma): source, license, known limitations, validation
                           metrics
```

**Hardcoded flags and thresholds specifically** (the highest-scrutiny part of this codebase):
- All thresholds (NEWS2 bands, GCS bands, burn %BSA/degree cutoffs) live in a single versioned config file (`lib/config/clinical_thresholds.dart` or equivalent JSON), **never scattered as magic numbers in business logic**.
...
### 5.2 Tier 2 (MedGemma) — output-contract and invariant tests
...
- Full vignette suite run at minimum before every release tag, with results archived (not just printed to a CI log) so historical comparisons are possible.

---

## 6. Getting Started

*(Fill in once initial setup is finalized — Flutter version and model download/setup instructions for the MedGemma GGUF.)*
...
2. Passing boundary and invariant tests (§5.1, §5.2).
