...
- What it is: a Flutter app that validates **MedGemma 1.5-4B** on-device via
  ...
  of a low-resource medical triage assistant (rule-based triage + MedGemma text
  reasoning; vision classifiers later dropped from scope — ADR-020).
...
- **Tier 1 (rule-based, no LLM):** GCS + NEWS2 + burn questions (UI)
  + hard red-flag overrides. Fully demo-able by itself.
- **Tier 2 (MedGemma text-only):** takes free-text history + vitals + burn
  answers + complaint probes and produces SOAP narrative. Reasons qualitatively
  about described symptoms (sore throat, eye redness, fatigue) from text.
- Rule: MedGemma can escalate, never downgrade Tier 1 (`max()` merge).
- Eye/Throat skin vision classifiers (originally planned, incl. a
  skin-condition classifier) were **dropped from scope** — see
...
assets/data/                     bundled DDI assets (ddi.json, drug_names.json)
```
...
> The vision classifier scope (skin/eye/throat) was **dropped** (ADR-020), so
> the earlier ML-side docs no longer reflect the shipped app. Current entry
> points are `docs/ARCHITECTURE.md`, `docs/DECISIONS.md`, and `docs/ROADMAP.md`.

### Dataset research highlights (verified in session; archived — vision was dropped)
...
- **AZH**: 6 classes verified. **SCIN**: skin dataset researched (archived with the vision path). **Oral Images**
...
- Build MedGemma-only Tier 2 prompt (text symptoms, vitals, complaint probes, burn).
...
- Researched/verified candidate vision datasets and wrote the merge plan
  (later archived: vision input dropped — ADR-020).
- Agreed the final architecture (Tier 1 + text-only MedGemma Tier 2).
